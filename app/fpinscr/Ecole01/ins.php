...
if(isset($_POST['captcha'])){
	...
	if($_POST['captcha']==$_SESSION['code']){
				...
				if(isset($_POST['submit'])){
							...
							$sqll='Select count(ID), ID from inscription_ecole where (upper(NOME)=upper("'.$nome.'"))and (upper(PRENOME)=upper("'.$prenome.'")) and (DATN="'.$datn.'") ;';
							$reqq = $con->query($sqll);
...
//							print $idd;
							...
							$sqll='Select * from inscription_ecole order by id desc ;';
							$reqq = $con->query($sqll);
							...
							if (isset($_FILES['file1'])) 
								{
											...
											
											{
												...
												// Copie dans le repertoire du script avec un nom
																	...
																	$repertoireDestination = $path."backup/ecole01/inscription/pieces/";
							...
							//preparation de la requete
								...
								$sql='INSERT INTO `inscription_ecole` (`ID`, `DAT`, `NOMA`, `PRENOMA`, `LIP`, `FONC`, `MATA`, `DIRA`, `ENTRA`, `ADRA`, `NPHA`, `NOME`, `PRENOME`, `NOMAR`, `PRENOMAR`, `SEXE`, `DATN`, `LIEU`, `ADR`, `MAIL`, `NPHE`,`NPHT`, `NPHF`, `ANNEE`, `NIVS`, `PHOTO`, `WILAYA_N`, `COMMUNE_N`) VALUES 
