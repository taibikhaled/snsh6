...
if(isset($_POST['captcha'])){
	...
	$url="";
			...
			$url=$url."&clieu=".$CLIEU;
							...
							$sqll=' SELECT * FROM equivalent_niveau_oran where id='.$nivs;
	...
	if($_POST['captcha']==$_SESSION['code']){
				...
				if(isset($_POST['submit'])){
							...
							$sqll='Select count(ID), ID from inscription_oran where (upper(NOME)=upper("'.$nome.'"))and (upper(PRENOME)=upper("'.$prenome.'")) and (DATN="'.$datn.'") ;';
							$reqq = $con->query($sqll);
...
//							print $idd;
							...
							$sqll='Select * from inscription_oran order by id desc ;';
							$reqq = $con->query($sqll);
							...
							if (isset($_FILES['file1'])) 
								{
											...
											
											{
												...
												// Copie dans le repertoire du script avec un nom
																	...
																	$repertoireDestination = $path."backup/oran/inscription/pieces/";
								...
								if (isset($_FILES['file2'])) 
								{
											...
											
											{
												...
												// Copie dans le repertoire du script avec un nom
																	...
																	$repertoireDestination = $path."backup/oran/inscription/photos/";
																	$nomDestination_f_img = "img".$idd.".".$extensionFichier;
							...
							//preparation de la requete
								...
								$sql='INSERT INTO `inscription_oran` (`ID`, `DAT`, `NOMA`, `PRENOMA`, `LIP`, `FONC`, `MATA`, `DIRA`, `ENTRA`, `ADRA`, `NPHA`, `NOME`, `PRENOME`, `NOMAR`, `PRENOMAR`, `SEXE`, `DATN`, `LIEU`, `ADR`, `MAIL`, `NPHE`,`NPHT`, `NPHF`, `ANNEE`, `NIVS`, `PHOTO`, `WILAYA_N`, `COMMUNE_N`) VALUES 
							...
							$_SESSION['id_inscription_oran'] = $idd;
