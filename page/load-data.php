<?php
    ...
    $sqll='Select * from message where from_id='.$id.' order by id_msg desc LIMIT  2 ;';
    ...
    $lien=$_SESSION['path']."app/Espace_membre/dashboard/messages/detaille_message/index.php?id=".$row['from_id']."&id_msg=".$row['id_msg'];
  ...
  //    if ($sexe=='M') $image=$_SESSION['path']."images/man.png";
  //    if ($sexe=='F') $image=$_SESSION['path']."images/woman.png";
    ...
    /*  echo '<div class="dropdown-menu dropdown-menu-right navbar-dropdown" aria-labelledby="profileDropdown">'; 
     echo ' 
      ...
      <img class="rounded-circle" src="'.$image.'" alt="" width="50%">
      <div class="text-truncate">'.$row['objet'].'</div>
      <div class="small text-gray-500">'.$row['nom'].'</div>
      </a>';
      echo '</div>'; */
