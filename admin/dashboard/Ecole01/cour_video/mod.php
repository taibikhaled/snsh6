...
$extensionsAutorisees =array("MP4", "mp4");
	...
	if ($test_pj ==true) $f_pj=$nomDestination_f_pj; else $f_pj='';
	  ...
	  $sql = "UPDATE cour_video SET  lib_doc=:titre, lien=:f_pj, etablissement=:cat,  cldem=:spe, codmat=:matiere, nom_ens=:nom, url_youtube=:url_youtube  WHERE id='".$idd."'";
	  $datas = array( ':titre'=>$titre,':f_pj'=>$f_pj,':cat'=>$cat, ':spe'=>$spe, ':matiere'=>$matiere, ':nom'=>$nom, ':url_youtube'=>$url_youtube  );
